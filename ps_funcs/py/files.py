...
def file_get_contents (filename_str:str, binary_mode_bool=False, forceZip=False, return_lst_bool:bool=False, encoding_str:str|None=_default_encoding)->str|list|bytes|None:
        """
        ...
        try:
                ...
                with open (filename_str, "rb" if binary_mode_bool else "rt", encoding=encoding_str) as inf:
                        if return_lst_bool:
                        	if is_binary_mode_bool:
                        		raise PSException ("Could not return list from gile_get_contents func in binary mode")
                        	else:
